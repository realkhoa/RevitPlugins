            ...
            try
            {
                ...
                if (currentDimId != ElementId.InvalidElementId && isFamilyDoc)
                {
                    ...
                    if (finalDim != null && finalDim.References.Size == 2)
                    {
                        ...
                        {
                            // 1. Lấy danh sách các Parameter hệ Length đang có sẵn trong Family
                            ...
                            // Vòng lặp: Nếu gán lỗi thì quay lại bắt chọn tiếp, trừ khi user bấm tắt form
                            while (!labelAssigned)
                            {
                                ...
                                {
                                    break; // User không nhập gì hoặc nhấn X tắt dialog -> Hủy gắn label
                                }
                                ...
                                using (Transaction tx = new Transaction(doc, "Add Label To DIM"))
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        // 2. Kiểm tra xem người dùng nhập tên mới hay chọn tên cũ
                                        foreach (FamilyParameter fp in doc.FamilyManager.Parameters)
                                        {
                                        ...
                                        tx.Commit();
                                        labelAssigned = true; // Gắn thành công, thoát vòng lặp
                                    }
                                    catch (Exception ex)
                                    {
                                        tx.RollBack();
                                        // Nếu bị lỗi (Vd: Chọn nhầm Text param, hoặc Param tạo ra vòng lặp ràng buộc quá mức)
